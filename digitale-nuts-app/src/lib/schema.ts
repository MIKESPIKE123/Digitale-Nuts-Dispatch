...
const inferIcon = (title: string) => {
  ...
  if (lower.includes('basis')) return '🗂️'
  if (lower.includes('plaats') || lower.includes('locatie')) return '📍'
  if (lower.includes('inspect')) return '👤'
  if (lower.includes('werkt') || lower.includes('partijen')) return '👥'
  if (lower.includes('synergie')) return '🔗'
  if (lower.includes('opmeting')) return '📏'
  if (lower.includes('veiligheid')) return '🚧'
  if (lower.includes('opbraak')) return '🏗️'
  if (lower.includes('sleuf')) return '🕳️'
  if (lower.includes('fundering')) return '🧱'
  if (lower.includes('legbed')) return '🪨'
  if (lower.includes('bestrating')) return '🧩'
  if (lower.includes('inzanding')) return '🪵'
  if (lower.includes('kasten')) return '📦'
  if (lower.includes('zuiverheid')) return '✨'
  return '📝'
}
...
export const parseArchisnapperCSV = (text: string): ParsedSchema => {
  ...
  const addField = (section: Section | null, label: string, hint?: string) => {
    ...
    if (!trimmed) return null
    const required = trimmed.toLowerCase().includes('verplicht') || currentSectionTitle.toLowerCase().includes('verplicht')
    const field: Field = {
  ...
  const addOptionOrNote = (field: Field | null, rawValue: string) => {
    ...
    field.options = field.options ? [...field.options, { value, label: value, isNOK, isOK }] : [{ value, label: value, isNOK, isOK }]
  }
  ...
  rows.forEach((row) => {
    ...
    if (col1 && col2) {
      const section = ensureSection(col1)
      const field = addField(section, col2, col3 || undefined)
      if (section?.title.toLowerCase().includes('inspecteurs') && field?.label.toLowerCase().includes('naam')) {
        currentSection = section
      }
      return
    }
  ...
  sections.forEach((section) => {
    section.items.forEach((field) => {
      ...
      fieldsByLabel[labelKey] = field
      fieldsByKey[field.key] = field
      if (section.title.toLowerCase().includes('inspecteurs') && field.label.toLowerCase().includes('naam') && field.options) {
